...
public class Note implements Serializable {
    ...
    // In this field we use the user's registration number.Now, why do we use the registration number instead of using a reference to the user, the answer is because of the serialization, when using a reference we are specifying so to speak a memory address, there would be no problem at first, but when loading the users from the file their memory locations may change and generate problems. then for more security we use the registration number, which we also ensure that it is unique foreach user registered in the system.
    ...
    public void setTags(List<String> tags) {
        ...
        // It is done in this way to avoid that this value can be modified without going through this method if not copied this way, we would just assign a reference to tags, so someone with that reference could modify it without us knowing, and put the save file and this object out of sync
