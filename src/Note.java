...
public class Note implements Serializable {
  ...
  // Now, why do we use the registration number instead of using a reference to the user, the answer
  // is because of the
  // serialization, when using a reference we are specifying so to speak a memory address, there
  // would be no problem
  // at first, but when loading the users from the file their memory locations may change and
  // generate problems. then
  // for more security we use the registration number, which we also ensure that it is unique for
  // each user
  ...
  public void setTags(List<String> tags) {
    ...
    // method
    // if not copied this way, we would just assign a reference to tags, so someone with that
    // reference could modify
  ...
  public String toString() {
    ...
    String output =
        "\n"
            ...
            + "\n\n"
            + "Tags       : "
            + OutputFormatter.formatTags(tags)
            + "\nColor      : "
            + color
            + "\n"
            + "Prioridad  : "
